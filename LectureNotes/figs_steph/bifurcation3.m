...
for m = 1 : mMax
    ...
    for k = 1:n
        x = r-x^2; % running n iterations to settle at a stationary regime
    end
    X(1) = x;
    for k = 1:n
        X(k+1) = r-X(k)*X(k); % running another n iterations to display a stationary regime
    end
...
title('Bifurcation diagram for the r-x^2');
