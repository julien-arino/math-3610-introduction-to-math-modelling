...
\begin{frame}\frametitle{Another interpretation of the logistic equation}
...
\end{equation}
\vfill
Such an equation is called a \defword{delay differential equation} (DDE). It is much more complicated to study than \eqref{eq:logistic_ode}. In fact, some things remain unknown about \eqref{eq:logistic_dde}
}
