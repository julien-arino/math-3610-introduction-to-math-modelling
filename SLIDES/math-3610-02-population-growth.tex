...
% For $N<N^*=0$, $N\pprime=bN-dN>0$, so $N(t)$ is increasing
% \vfill
% For $N>N^*=0$, $N\pprime=bN-dN<0$, so $N(t)$ is decreasing
% \vfill
% This means that $N^*=0$ is an \defword{unstable equilibrium}
% \vfill
% \begin{center}
% \begin{tikzpicture}
% \begin{axis}[xlabel=$N$,ylabel=$N\pprime$,axis lines=middle,grid=major]
% \addplot[domain=-2:2,samples=100]{x*(1-x)};
% \end{axis}
% \end{tikzpicture}
% \end{center}
% \end{frame}
...
\frame{\frametitle{Obtaining the well-known form \eqref{eq:logistic}}
\begin{align*}
N\pprime &= (b-d)N-cN^2\\
&= \bigl((b-d)-cN\bigr)N \\
...
\frame{
\defword{Equilibria} of \eqref{eq:logistic_ode} are points such that $f(N)=0$ (so that $N'=f(N)=0$, meaning $N$ does not vary). So we solve $f(N)=0$ for $N$. We find two points:
\begin{itemize}
\item $N=0$
\item $N=K$
\end{itemize}
